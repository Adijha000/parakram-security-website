...
def art(ik,m,cls=''):
    return f'<div class="art m{m}{(" "+cls) if cls else ""}"><div class="rg a"></div><div class="rg b"></div><span class="ai">{IC[ik]}</span></div>'
...
def pbn(crumbs,h1,lead,bg,pos,ctas,meta=None):
    ...
    if bg.startswith('art:'):
        _,ik,mt=bg.split(':'); pbgd=f'<div class="pbg pbart">{art(ik,mt)}</div>'
    else: pbgd=f'<div class="pbg" style="--bg:url({bg});--pos:{pos}"></div>'
    return f'''<section class="pbn"><div class="floor" aria-hidden="true"></div>{pbgd}<div class="wrap pbi"><div class="bc" role="navigation" aria-label="Breadcrumb">{bc}</div><h1>{h1}</h1><p class="lead">{lead}</p><div class="cta">{ctas}</div>{('<div class="pm2">'+ms+'</div>') if ms else ''}</div></section>'''
...
def related(exclude=None):
    ...
    for s in SERV:
        ...
        o+=f'<a class="rcard rv" href="/services/{s[0]}"><div class="rt">{art(s[2],(SERV.index(s)%8)+1)}</div><div class="rb"><b>{s[1].replace("&","&amp;")}</b><span>{s[6]}</span><em>Learn more →</em></div></a>'
...
def cov24():
    bars=''.join('<span></span>' for _ in range(24))
    return f'<div class="cov24 rv" id="cov24"><div class="ctabs"><button data-m="day">Day shift</button><button data-m="night">Night shift</button><button class="on" data-m="full">24/7 coverage</button></div><div class="bar24">{bars}</div><div class="lab24"><span>12 AM</span><span>6 AM</span><span>12 PM</span><span>6 PM</span></div><p id="cap24"></p></div>'
def qcard(nm):
    return f'<aside class="sva"><div class="qc rv"><h3>Get a plan for {nm}</h3><p class="s">Tell us who you are and we will call you back with a plan customized to your site.</p><form data-s="{nm}"><input name="n" placeholder="Your name" autocomplete="name" aria-label="Your name"><input name="p" type="tel" placeholder="Phone number" autocomplete="tel" aria-label="Phone number"><button class="btn btn-g" type="submit">Request a callback →</button></form><a class="ln" href="tel:+919105909006">{IC["ph"]}+91 91059 09006</a><a class="ln" href="mailto:info@parakramindia.org">{IC["ml"]}info@parakramindia.org</a><a class="ln" href="https://wa.me/919105909006" target="_blank" rel="noopener">{IC["ph"]}Chat on WhatsApp</a><p class="fine">Available 24/7. No obligation.</p></div></aside>'
def trustbar():
    it=[('usr','Trained and verified','Every person, before deployment'),('clk','Available 24/7','Around the clock service'),('eye','Customized to your site','Planned around your risk'),('star','4.9 client rating','From 1,200+ satisfied clients')]
    return '<div class="tbar"><div class="wrap">'+''.join(f'<div class="tbi"><i>{IC[i]}</i><div><b>{t}</b><span>{d}</span></div></div>' for i,t,d in it)+'</div></div>'
def glance(a,b,c):
    return '<div class="glance">'+''.join(f'<div><b>{x}</b><span>{y}</span></div>' for x,y in (a,b,c))+'</div>'
def expect_band(eyebrow='What you can expect',h='Security you can <em>rely on.</em>'):
    items=[('01','Verified people','Every person is verified and trained before they reach your site.'),('02','Clear instructions','Written post orders so that every duty and escalation step is defined.'),('03','Regular supervision','Supervisors and managers visit posts and act quickly on feedback.'),('04','A plan that adapts','We review with you and adjust as your site and risks change.')]
    g=''.join(f'<div class="xpc"><span class="k">{a}</span><b>{b}</b><p>{c}</p></div>' for a,b,c in items)
    return f'<section class="blk xp"><div class="wrap">{sech(eyebrow,h,"Good security is a habit, not a product. These four habits shape every deployment we run.")}<div class="xpg rv">{g}</div><div class="xpn rv"><div><b data-n="9" data-s="+">0</b><span>Years of experience</span></div><div><b data-n="1500" data-s="+">0</b><span>Trained professionals</span></div><div><b>24/7</b><span>Availability</span></div><div><b data-n="4.9" data-d="1">0</b><span>Client rating</span></div></div></div></section>'
def paths_grid():
    it=[('ind','Factory or warehouse','Industrial security','/industries#industrial'),('com','Office or business','Corporate security','/industries#corporate'),('hos','Hospital or clinic','Healthcare security','/industries#healthcare'),('bnk','Bank or ATM','Banking security','/services/banking'),('res','Home or society','Residential security','/services/residential'),('edu','School or college','Educational security','/services/educational')]
    return '<div class="paths">'+''.join(f'<a class="pth rv" href="{u}"><i>{IC[i]}</i>{t}<span>{d}</span></a>' for i,t,d,u in it)+'</div>'
...
# ---------- Services overview ----------
def services_page():
    o=pbn([('Services',None)],'Eight services. <em>One standard.</em>','Customized security solutions for every environment, delivered by trained and verified personnel who are available 24/7.','art:arm:3','50% 20%',BTN('Plan Your Security →','/planner')+BTN('Talk to Us','/contact','btn-o'),['Trained and verified','Available 24/7','Customized to your site'])
    cs=''
    for s in SERV:
...
EXP={
'manned':dict(insight='Most security failures are not dramatic. They happen at an unattended gate, a missed patrol or a shift handover where information is lost. Guarding works when every post has clear written instructions, every guard knows exactly what is expected, and someone senior checks that it is happening. That discipline, more than headcount, is what protects a site.',
 risks=[('Unauthorised entry','Weak gate procedures let unknown people, vehicles and material move in and out unchecked.'),('Shift handover gaps','Information lost between shifts is one of the most common causes of repeat incidents.'),('Fatigue and complacency','Long duties without supervision lead to lapses. Rotation and regular checks keep guards alert.'),('Slow escalation','An incident that is not reported quickly is harder to contain and harder to learn from.')],
 good=['Every post has written instructions that the guard can explain in their own words.','Gate registers and patrol records are complete and easy for you to review.','Supervisors visit at varied times, including at night.','Concerns you raise are answered quickly and closed properly.'],
 faq=[('What is a post order?','A post order is a written set of instructions for a specific post. It covers duties, timings, who to call and what to do in common situations. Clear post orders are the foundation of consistent guarding.'),('What happens if a guard is absent?','Reliefs are planned in advance so that a post is never left empty. If a guard is unavailable, a trained replacement is arranged and briefed on the post.')]),
'armed':dict(insight='Armed deployment is a serious responsibility, not simply an upgrade. It calls for the right legal framework, carefully selected personnel, strict rules on when and how a weapon may be handled, and constant supervision. Our approach is to start with the risk, decide whether armed cover is truly needed, and then build a deployment that is lawful, disciplined and proportionate.',
 risks=[('Licence and legal compliance','Armed cover must follow the rules that apply to your site and location. Gaps here create liability for everyone involved.'),('Improper handling','Poor discipline around weapons is a risk in itself. Selection, training and supervision must be strict.'),('Over reliance on arms','A weapon is not a substitute for good access control, patrols and awareness. It works best as one layer of a wider plan.'),('Unclear rules of conduct','Personnel need clear instructions on escalation, restraint and reporting so that decisions are consistent under pressure.')],
 good=['The need for armed cover is justified by a documented risk assessment.','Personnel are verified, trained and supervised to a defined standard.','Rules of conduct and reporting are written down and briefed.','Armed and unarmed staff work to one coordinated plan.'],
 faq=[('How do you decide whether armed cover is necessary?','We begin with a risk assessment of your assets, location and threat profile. If armed cover is not needed, we will say so and recommend a more suitable plan.'),('How is discipline maintained around weapons?','Through careful selection, clear written rules, regular briefings and close supervision. Discipline is treated as a daily habit, not a one time event.')]),
'industrial':dict(insight='Industrial sites are large, busy and full of moving parts. The real risks are usually pilferage, uncontrolled vehicle and material movement, and blind spots along long perimeters, rather than dramatic intrusions. Good industrial security is built on strong gate control, reliable records and patrols that are planned around how your shifts actually run.',
 risks=[('Material and vehicle leakage','Goods leaving without proper authorisation is the most common loss at industrial sites.'),('Perimeter blind spots','Long boundaries, storage yards and dark corners need planned patrols and clear reporting.'),('Shift pattern gaps','Sites are most exposed at shift changes, during breaks and after hours.'),('Safety and security overlap','Guards are often the first to notice hazards, so they need to understand your safety rules.')],
 good=['Every vehicle and consignment is checked against a clear authorisation process.','Patrol routes cover the whole perimeter and are varied so they are not predictable.','Records let you reconstruct who was where, and when.','Guards know your safety rules and raise hazards quickly.'],
 faq=[('How do you reduce pilferage?','By tightening gate procedures, matching material movement to authorisation, keeping accurate records and varying patrols so that patterns cannot be exploited.'),('Can you work with our existing gate systems?','Yes. We fit our procedures to the systems you already use and suggest improvements where they would help.')]),
'commercial':dict(insight='In an office or business building, security is part of your brand. The guard at reception is often the first person a client meets. The best commercial security is quiet and confident: it keeps unauthorised people out, keeps your staff and assets safe, and makes every visitor feel welcome. That balance depends on training, grooming and courtesy as much as vigilance.',
 risks=[('Tailgating and weak access control','Unauthorised people following staff through doors is the most common way premises are breached.'),('After hours exposure','Empty buildings need patrols and alert monitoring of entrances.'),('Visitor friction','A slow or unfriendly entry process reflects on your organisation.'),('Shared building complexity','Multi tenant sites need consistent rules that everyone understands.')],
 good=['Visitors are greeted, checked and directed smoothly.','Access rules are applied consistently to everyone, including senior staff.','Guards are well presented and courteous at all times.','Building management and tenants know who to call and get fast answers.'],
 faq=[('How do you balance security with a welcoming reception?','Through training and clear procedures. Guards are taught to be firm on process and warm in manner, so that checks feel professional and not intrusive.'),('Can you adapt to our dress code and brand standards?','Yes. We can agree presentation standards with you and brief guards accordingly.')]),
'residential':dict(insight='A residential society runs on trust. Residents want to feel safe without feeling policed, and committees want rules applied fairly. The guard at the gate is the face of the community. Strong residential security therefore combines firm visitor control with genuine courtesy, and a supervisor who is easy to reach when residents have a concern.',
 risks=[('Unverified visitors and deliveries','Entry by unknown people is the most frequent security concern in societies.'),('Night time vulnerability','Common areas, parking and boundaries need planned patrols after dark.'),('Inconsistent rules','When guards apply rules differently, residents lose confidence and disputes follow.'),('Slow response to concerns','Residents need to know their concern has been heard and acted on.')],
 good=['Every visitor is verified in line with your society rules.','Guards greet residents politely and know the regular routines of the community.','Night patrols are logged and reviewed.','Committee feedback leads to visible improvements.'],
 faq=[('How do you handle a resident who is unhappy with a guard?','Concerns go to the supervisor, who follows up with the resident and the guard. Repeated issues are reviewed with the committee and corrected.'),('Can you help us set clear visitor rules?','Yes. We can suggest a simple, fair visitor and delivery process and brief guards to apply it consistently.')]),
'hospital':dict(insight='A hospital is one of the most demanding places to provide security. Visitors are anxious, emergencies arrive without warning, and access must stay open for those who need urgent care. Good hospital security is calm, patient and alert. Guards need the judgement to manage crowds gently, protect staff and keep emergency routes clear.',
 risks=[('Crowding at peak times','Visiting hours and emergencies create pressure at entrances and corridors.'),('Blocked emergency access','Vehicles and crowds at the emergency entrance can delay urgent care.'),('Staff safety','Tense situations can escalate. Staff need visible, trained support.'),('Sensitive areas','Wards, pharmacies and records areas need controlled access.')],
 good=['Emergency entrances and ambulance routes are always clear.','Guards communicate with patience and respect, even under pressure.','Restricted areas are controlled without disturbing patient care.','Staff know how to summon help quickly.'],
 faq=[('How do guards handle distressed families?','With patience, clear communication and respect. Our people are trained to stay calm, listen and involve the right member of hospital staff.'),('Do you coordinate with hospital administration?','Yes. We agree procedures with your administration and adjust them as your operations change.')]),
'educational':dict(insight='Schools and colleges need security that protects students without turning the campus into a fortress. The most important moments are arrival and dispersal, when large numbers of young people and vehicles converge on a few gates. Good campus security is visible, approachable and consistent, and it understands the rhythm of the academic day.',
 risks=[('Arrival and dispersal congestion','Peak times put students and vehicles in close contact.'),('Unauthorised visitors','Campuses need to know who is on site and why.'),('Unsupervised areas','Boundaries, sports grounds and less used buildings need patrols.'),('Event days','Admissions, examinations and functions bring crowds that need planning.')],
 good=['Gates run smoothly at the busiest times of day.','Guards are approachable to students, parents and staff.','Visitors are logged and directed properly.','Event plans are agreed in advance with the administration.'],
 faq=[('How do you support school opening and closing times?','We add personnel at peak times, control vehicle flow and guide students safely to and from the gates.'),('Can you support examinations and admissions?','Yes. We plan additional cover and crowd management for these events with your administration.')]),
'banking':dict(insight='Banks and ATMs depend on public trust. Customers expect a calm, orderly environment, and staff expect the premises to be watched carefully. Security here is about vigilance, procedure and presence: alert entrances, well briefed guards and clear steps to follow if something looks wrong. Verification of personnel matters more here than almost anywhere else.',
 risks=[('ATM tampering and misuse','Unattended or poorly watched ATMs attract fraud and interference.'),('Branch entrance risks','Entrances must be watched without unsettling customers.'),('Insider risk','Verification of every person on the payroll is essential at financial sites.'),('Delayed alerts','Guards need simple procedures for raising an alarm quickly.')],
 good=['Every guard is verified and briefed before deployment.','Entrances and ATM areas are watched throughout opening hours.','Alert procedures are simple, written and rehearsed.','Customers experience a calm, courteous environment.'],
 faq=[('How are guards for financial sites selected?','With extra care. Personnel are verified and trained before deployment and are supervised regularly.'),('Can you standardise procedures across many branches?','Yes. We build one set of procedures and apply it across branches so that customers and staff get the same experience everywhere.')]),
}
PLAN=[('Site survey','We walk your premises with you, study entry points, layouts and how the site is used through the day.'),('Risk assessment','We identify what could go wrong, how likely it is and what matters most to you.'),('Post orders','We write clear instructions for every post so that duties and escalation are never left to guesswork.'),('Staffing and shifts','We design the number of personnel, shift patterns and reliefs around your risk and budget.'),('Briefing and training','Every person is briefed on your site, your rules and your expectations before they start.'),('Supervision and review','Regular supervision, clear reporting and periodic reviews keep the plan working as your needs change.')]
for slug,name,ic,im,pos,lead,short,what,where,faqs in SERV:
    nm=name.replace('&','&amp;'); low=nm.lower().replace('&amp;','and'); E=EXP[slug]
    banner=pbn([('Services','/services'),(nm,None)],nm.replace(' Security',' <em>Security</em>'),lead,{'manned':img('operations'),'armed':img('hero-portrait'),'industrial':img('training')}.get(slug,f'art:{ic}:{[x[0] for x in SERV].index(slug)+1}'),pos,BTN('Get a Plan →','#getplan')+BTN('Talk to Us','/contact','btn-o'),['Trained and verified','Available 24/7','Customized to your site'])
    banner=banner.replace('</section>',glance(('24/7','Availability'),('100%','Verified personnel'),('1 plan','Customized to you'))+'</section>',1)
    sub='<div class="subnav"><div class="wrap"><div class="snv"><a href="#overview">Overview</a><a href="#covers">What it covers</a><a href="#risks">Risks we plan for</a><a href="#shifts">Shifts</a><a href="#planning">Planning</a><a href="#faqs">FAQ</a></div><a class="btn btn-g sm" href="#getplan">Get a plan</a></div></div>'
    main=(f'<section class="blk" id="overview"><div class="sp2t rv"><div class="eyebrow dk">Overview</div><h2 class="h2">{nm} that you can rely on.</h2><p>{lead}</p><p>{E["insight"]}</p>{ticks(["Trained and verified personnel","Available 24/7","Customized to your site"])}</div></section>'
     f'<section class="blk" id="covers">{sech("What it covers",f"What our {low} team does.","The exact scope is confirmed with you after a site assessment and adjusted as your needs change.")}{cards([(ic,a,b) for a,b in what])}</section>'
     f'<section class="blk" id="risks">{sech("Risks we plan for","Where things go wrong, and how we prevent it.","Every site has its own weak points. These are the ones we look for first.")}{cards([(None,a,b) for a,b in E["risks"]],"c2",num=True)}</section>'
     f'<section class="blk" id="shifts">{sech("Shift options","Choose the coverage that fits your site.","Pick a pattern to see how guards are scheduled across the day. We tailor the final pattern to your needs.")}{cov24()}</section>'
     f'<section class="blk" id="planning">{sech("How we plan a deployment","Six steps from first visit to daily control.","This is the method we follow for every new site, large or small.")}{cards([(None,a,b) for a,b in PLAN],"c2",num=True)}</section>'
     f'<section class="blk" id="standards"><div class="sp2t rv"><div class="eyebrow dk">What good looks like</div><h2 class="h2">The standard you should expect.</h2><p>These are the signs of a well run deployment. Hold us to them.</p>{ticks(E["good"])}<p style="margin-top:18px">We agree the reporting that suits you, for example daily activity records, incident reports and periodic reviews with your team.</p></div></section>'
     f'<section class="blk" id="where">{sech("Where it is used","Built for real environments.")}<div class="tiles rv">'+''.join(f"<div class='tile'>{w}</div>" for w in where)+'</div></section>'
     f'<section class="blk" id="faqs">{sech("FAQ",f"Questions about {low}.","Cannot find your answer? Call +91 91059 09006 or email info@parakramindia.org.")}<div class="rv">{faq(faqs+E["faq"])}</div></section>')
    body=banner+trustbar()+sub+f'<div class="wrap"><div class="svl"><div class="svm">{main}</div><div id="getplan">{qcard(nm)}</div></div></div>'
    body+=expect_band()
    body+=f'<section class="blk alt"><div class="wrap">{sech("Related services","Explore other services.")}{related(slug)}</div></section>'
    body+=cta2(f'Need {low} for your site?','Tell us about your premises and we will design a plan around it.')
    PAGES['services/'+slug]=(f'{name} | Parakram Security India',f'{short} Trained, verified personnel, available 24/7. Customized to your site.',body,'/services',faq_ld(faqs+E["faq"]))
...
def about_page():
    ...
    o+=trustbar()
    o+=f'''<section class="blk"><div class="wrap sp2"><div class="sp2t rv"><div class="eyebrow dk">Who we are</div><h2 class="h2">Protecting people, property and operations.</h2><p>We combine disciplined manpower, rigorous training and deeply rooted Indian values to protect people, property and operations. Our personnel are known for professionalism, integrity and courtesy.</p><p>Your safety is our mission. That mission shapes how we recruit, how we train and how we supervise every post we manage.</p><div class="cta" style="margin-top:26px">{BTN("Meet Our Services →","/services","btn-n")}</div></div><div class="sp2i art m3 rv"><div class="rg a"></div><div class="rg b"></div><div class="emb3d" aria-hidden="true"></div><span class="badge">Established 2017</span></div></div></section>'''
...
# ---------- Why ----------
def why_page():
    o=pbn([('Why Parakram',None)],'Why leaders choose <em>Parakram.</em>','Experience, trained and verified personnel, and solutions customized to your site.','art:star:2','50% 20%',BTN('Plan Your Security →','/planner')+BTN('Contact Us','/contact','btn-o'))
    o+=trustbar()
...
<div class="fcard rv"><span class="no2">100%</span><h3>Customized solutions</h3><p>Services designed around your site, your risk and your operations.</p></div></div></div></section>'''
    o+=f'<section class="blk"><div class="wrap">{sech("How we think about security","Three principles behind every deployment.","Technology and equipment help, but the quality of a security operation is decided by people and process.")}{cards([(None,"People before technology","Cameras and alarms only work when trained people respond well. We invest first in recruitment, training and supervision."),(None,"Process before presence","A guard standing at a gate is not security by itself. Clear post orders, records and escalation make presence effective."),(None,"Supervision before promises","We would rather show you how a post is supervised than promise perfection. Regular checks are how standards are kept.")],"",num=True)}</div></section>'
...
# ---------- Industries ----------
IND=[('industrial','Industrial','ind','training','50% 25%','Plants, warehouses and industrial campuses lose most to leakage of material, uncontrolled vehicle movement and gaps at shift changes. We plan gate control, patrols and records around how your operation actually runs, and we make sure guards understand your safety rules as well as your security rules.',['Gate and perimeter control','Vehicle and material checks','Shift wise deployment','Patrols across large areas'],[('industrial','Industrial Security'),('armed','Armed Security'),('manned','Manned Security')]),
('corporate','Corporate','com','operations','65% 25%','In corporate premises, security is part of your brand. We combine firm access control with a courteous front of house presence, so that visitors feel welcome and unauthorised entry is stopped quietly and consistently.',['Reception and lobby presence','Visitor and access management','Parking and vehicle control','After hours protection'],[('commercial','Commercial Security'),('manned','Manned Security')]),
('healthcare','Healthcare','hos','about','60% 20%','Healthcare sites need security that is calm, patient and alert. Emergency access must stay clear, visitors must be managed with empathy and staff must feel supported. Our guards are trained for exactly that balance.',['Visitor and crowd management','Emergency entrance access','Respectful, patient conduct','Round the clock coverage'],[('hospital','Hospital Security'),('manned','Manned Security')]),
('banking','Banking','bnk','operations','50% 30%','For banks and ATMs, trust is the product. We provide verified personnel, watchful entrances and simple alert procedures, applied consistently across branches so that customers and staff get the same experience everywhere.',['Branch entrance security','ATM vigilance','Verified personnel','Clear alert procedures'],[('banking','Bank and ATM Security'),('armed','Armed Security')])]
def industries_page():
    o=pbn([('Industries',None)],'Trusted where safety <em>matters most.</em>','Industrial, corporate, healthcare and banking clients rely on Parakram for trained and verified personnel.','art:ind:5','50% 20%',BTN('Plan Your Security →','/planner')+BTN('Talk to Us','/contact','btn-o'))
    ...
    for k,(sl,nm,ic,im,pos,txt,pts,links) in enumerate(IND):
        ...
        o+=f'''<section class="blk{alt}" id="{sl}"><div class="wrap sp2{rev}"><div class="sp2t rv"><div class="eyebrow dk">0{k+1} / {nm}</div><h2 class="h2">Security for {nm.lower()} sites.</h2><p>{txt}</p>{ticks(pts)}<div style="margin-top:22px">{ls}</div></div><div class="sp2i art m{k*2+1} rv"><div class="rg a"></div><div class="rg b"></div><span class="ai">{IC[ic]}</span><span class="badge">{nm}</span></div></div></section>'''
...
# ---------- Coverage ----------
def coverage_page():
    o=pbn([('Coverage',None)],'Local roots. <em>Wider reach.</em>','Headquartered in Haridwar, Uttarakhand, with support for multiple sites.','art:map:6','50% 20%',BTN('Tell Us Your Locations →','/contact')+BTN('Plan Your Security','/planner','btn-o'))
    o+=trustbar()
...
# ---------- Careers ----------
def careers_page():
    o=pbn([('Careers',None)],'Serve with <em>Parakram.</em>','Join a disciplined, professionally managed team built on integrity and courtesy.','art:usr:7','50% 20%',BTN('Apply Now →','#apply')+BTN('Email Us','mailto:info@parakramindia.org?subject=Career%20enquiry','btn-o'))
    o+=trustbar()
    ...
    o+=f'<section class="blk alt"><div class="wrap sp2"><div class="sp2t rv"><div class="eyebrow dk">Who we look for</div><h2 class="h2">People ready to learn and serve.</h2><p>Parakram recruits with care. If you are disciplined, honest and want to build a career in security, we would like to hear from you.</p>{ticks(["Discipline and a sense of responsibility","Honesty and integrity","Courtesy towards everyone you serve","Willingness to complete rigorous training","Readiness to work in shifts"])}</div><div class="sp2i art m6 rv"><div class="rg a"></div><div class="rg b"></div><span class="ai">{IC["book"]}</span><span class="badge">Join our team</span></div></div></section>'
    o+=f'<section class="blk"><div class="wrap">{sech("Life at Parakram","Discipline, training and teamwork.","Our people train, serve and grow together.")}<div class="mos rv"><div class="big"><div class="art m3" style="position:absolute;inset:0"><div class="rg a"></div><div class="rg b"></div><span class="ai">{IC["book"]}</span></div><b>Rigorous training</b></div><div><div class="art m1" style="position:absolute;inset:0"><div class="rg a"></div><div class="rg b"></div><span class="ai">{IC["arm"]}</span></div><b>Discipline</b></div><div><div class="art m5" style="position:absolute;inset:0"><div class="rg a"></div><div class="rg b"></div><span class="ai">{IC["chk"]}</span></div><b>Verified personnel</b></div><div><div class="art m7" style="position:absolute;inset:0"><div class="rg a"></div><div class="rg b"></div><span class="ai">{IC["usr"]}</span></div><b>One team</b></div><div><div class="art m2" style="position:absolute;inset:0"><div class="rg a"></div><div class="rg b"></div><span class="ai">{IC["star"]}</span></div><b>Courtesy</b></div></div></div></section>'
    ...
    o+=f'''<section class="blk alt" id="apply"><div class="wrap sp2" style="align-items:start"><div class="sp2t rv"><div class="eyebrow dk">Apply</div><h2 class="h2">Start your application.</h2><p>Share a few details and upload your resume. Our team reviews every application and contacts suitable candidates.</p>{ticks(["Upload a PDF or Word resume, up to 3 MB","Your details go only to the Parakram team","Prefer email? Write to info@parakramindia.org"])}</div><form class="frm rv" id="cvf" novalidate><div><label for="vn">Full name</label><input id="vn" required autocomplete="name"></div><div><label for="vp">Phone</label><input id="vp" type="tel" required autocomplete="tel"></div><div><label for="ve">Email (optional)</label><input id="ve" type="email" autocomplete="email"></div><div><label for="vc">City</label><input id="vc" autocomplete="address-level2"></div><div class="full"><label for="vr">Role of interest</label><select id="vr"><option>Security guard</option><option>Supervisor</option><option>Armed guard</option><option>Other</option></select></div><div class="full"><label for="vm">A little about you</label><textarea id="vm" placeholder="Your experience, if any"></textarea></div><div class="full"><label>Resume</label><div class="drop" id="drop"><input id="vf" type="file" accept=".pdf,.doc,.docx,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" aria-label="Upload your resume"><svg viewBox="0 0 24 24"><path d="M12 16V4M7 9l5-5 5 5"/><path d="M4 16v3a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1v-3"/></svg><b>Drop your resume here or click to browse</b><span>PDF or Word, up to 3 MB</span></div><div class="fileok" id="fileok" style="margin-top:10px"><span></span><button type="button">Remove</button></div></div><input class="hp" id="vh" tabindex="-1" autocomplete="off" aria-hidden="true"><div class="full cvst" id="cvst" role="status"></div><button class="btn btn-n full" id="cvb" style="justify-content:center">Submit application</button></form></div></section>'''
...
def faq_page():
    o=pbn([('FAQ',None)],'Answers <em>before you ask.</em>','Everything you need to know about our services, our people and how we work.','art:book:8','50% 20%',BTN('Ask Us Directly →','/contact')+BTN('Plan Your Security','/planner','btn-o'))
...
def contact_page():
    o=pbn([('Contact',None)],'Let us secure <em>your world.</em>','Speak to our team about a security plan customized to your site. We are available 24/7.','art:ph:1','50% 20%',BTN('Plan Your Security →','/planner')+BTN('Call Now','tel:+919105909006','btn-o'))
    o+=trustbar()
...
# ---------- Planner ----------
def planner_page():
    o=pbn([('Security Planner',None)],'Tell us what you need. <em>We will design the plan.</em>','Answer four quick questions and our team will respond with a solution customized to your site, with no obligation.','art:eye:4','50% 20%','',['Takes about a minute','Opens on WhatsApp','No obligation'])
